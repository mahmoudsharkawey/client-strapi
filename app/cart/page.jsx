...
const Cart = () => {
  ...
  const deletCartItem = (id) => {
    CartApis.deletUserCartItems(id)
      .then((res) => {
        if (res)
          setCart((oldCart) =>
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-8">
                ...
                {cart?.map((item) => (
                  <li key={item?.id} className="flex items-center gap-4">
                    ...
                    <div className="flex flex-1 items-center justify-end gap-7">
                      ...
                      <button
                        onClick={(id) => deletCartItem(item?.id)}
                ...
                <div className="w-screen max-w-lg space-y-4">
                  <dl className="space-y-0.5 text-sm text-gray-700">
                    <div className="flex justify-between !text-base font-medium">
                      <dt className="text-[25px]">Total</dt>
                      <dd className="text-[25px]">{getTotalAmount()}</dd>
                    </div>
                  </dl>
              ...
              <h2 className="text-gray-400 text-[19px]">
                Note:
                <br /> All Items will be sent via Email
              </h2>
            </div>
          </div>
        </div>

...
function ProductInfo({ product }) {
  ...
  return (
    <div>
      {product?.id ? (
        <div>
          <h1 className="text-[20px] pb-5">{product?.attributes?.title} </h1>
          ...
          <h2 className="text-[24px] text-primary mt-2 pb-5">
            $ {product?.attributes?.price}
          </h2>
          <button
            onClick={() => handleAddToCart()}
            className="flex gap-2 p-2 text-white rounded-lg bg-primary hover:bg-[#626161]"
          >
